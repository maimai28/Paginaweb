...
@guia('PF15 Movimiento armónico simple')
def _():
    g = 9.8
    w = math.sqrt(200/0.5)
    return [
        ('ejemplo1', (w, R_(2*math.pi/w, 3), R_(w/(2*math.pi)), 0.05*w, R_(0.05*w**2, 6)), (20.0, 0.314, 3.18, 1.0, 20.0)),
        ('ejemplo2', (R_(0.2*g/0.05, 6), R_(2*math.pi*math.sqrt(0.2/39.2), 3)), (39.2, 0.449)),
        ('ejemplo3', (R_(0.5*200*0.05**2, 6), R_(w*math.sqrt(0.05**2 - 0.03**2), 6)), (0.25, 0.8)),
        ('ejemplo4', (R_(2*math.pi*math.sqrt(1/g)), R_(g*4/(4*math.pi**2), 3)), (2.01, 0.993)),
        ('ej1', (math.sqrt(800/2), R_(2*math.pi/20, 3), R_(20/(2*math.pi))), (20.0, 0.314, 3.18)), ('ej2', 2**2, 4),
        ('ej3', (0.1*20, R_(0.1*400, 6)), (2.0, 40.0)), ('ej4', (R_(0.5*800*0.01, 6), R_(20*math.sqrt(0.01 - 0.0025))), (4.0, 1.73)),
        ('ej5', (R_(0.5*g/0.1, 6), R_(2*math.pi*math.sqrt(0.5/49), 3)), (49.0, 0.635)),
        ('ej6', (R_(2*math.pi/5), R_(0.2*math.cos(2.5), 3)), (1.26, -0.160)),
        ('ej7', R_(2*math.pi*math.sqrt(2.5/g)), 3.17), ('ej8', R_(g*1.5**2/(4*math.pi**2), 3), 0.559),
        ('ej9', R_(2*math.pi*math.sqrt(1/1.62)), 4.94),
    ]

@guia('PF16 Ondas mecánicas')
def _():
    db = lambda I: 10*math.log10(I/1e-12)
    return [
        ('cuerda', (R_(0.012/1.5, 6), math.sqrt(80/0.008), R_(100/3, 1)), (0.008, 100.0, 33.3)),
        ('ejemplo1', round(343*1.2/2), 206), ('db', (R_(10*math.log10(2), 1), R_(db(1e-5), 6)), (3.0, 70.0)),
        ('ejemplo3', (round(700*343/(343 - 25)), round(700*343/(343 + 25))), (755, 652)),
        ('ej1', 1.5*2, 3.0), ('ej2', R_(343/440), 0.78), ('ej3', 343*3, 1029), ('ej4', (R_(2*0.65*110, 6), 2*110), (143.0, 220)),
        ('ej5', math.sqrt(4), 2.0), ('ej6', R_(db(1e-3), 6), 90.0), ('ej7', 10**((80 - 60)/10), 100.0),
        ('ej8', (343/(4*0.25), 3*343/(4*0.25)), (343.0, 1029.0)), ('ej9', round(500*343/(343 + 30)), 460),
        ('ej10', round(400*(343 + 20)/343), 423),
    ]

@guia('PF17 Reflexión y refracción')
def _():
    sn = lambda a_: math.sin(rad(a_))
    return [
        ('vidrio', 3e8/1.5, 2e8), ('ejemplo1', (R_(sn(40)/1.33, 3), R_(deg(math.asin(sn(40)/1.33)), 1)), (0.483, 28.9)),
        ('ejemplo2', (R_(deg(math.asin(1/1.5)), 1), R_(deg(math.asin(1/2.42)), 1)), (41.8, 24.4)),
        ('aparente', R_(2/1.33, 1), 1.5),
        ('ej1', (35, 90 - 35), (35, 55)), ('ej2', R_(3e8/2.42/1e8), 1.24), ('ej3', R_(3e8/2.25e8), 1.33),
        ('ej4', R_(deg(math.asin(sn(30)/1.5)), 1), 19.5), ('ej5', R_(deg(math.asin(1.33*sn(30))), 1), 41.7),
        ('ej6', (1.33*sn(60) > 1, R_(deg(math.asin(1/1.33)), 1)), (True, 48.8)), ('ej7', R_(deg(math.asin(1/2.42)), 1), 24.4),
        ('ej8', R_(1.2/1.33), 0.90), ('ej9', (round(500/1.5), 3e8/500e-9), (333, 6e14)),
    ]

@guia('PF18 Espejos y lentes')
def _():
    di = lambda f_, do: 1/(F(1, 1)/f_ - F(1, 1)/do) if F(1, 1)/f_ != F(1, 1)/do else None
    m_ = lambda f_, do: -di(f_, do)/do
    return [
        ('ejemplo1', (di(10, 30), m_(10, 30)), (15, F(-1, 2))), ('ejemplo2', (di(10, 5), m_(10, 5)), (-10, 2)),
        ('ejemplo3', (di(-20, 20), m_(-20, 20)), (-10, F(1, 2))), ('ejemplo4', (di(15, 40), m_(15, 40)), (24, F(-3, 5))),
        ('potencia', 1/0.5, 2.0),
        ('ej1', (2 + 2, 0.5 + 0.5), (4, 1.0)), ('ej2', (di(20, 60), m_(20, 60)), (30, F(-1, 2))), ('ej3', di(20, 20), None),
        ('ej4', (di(20, 10), m_(20, 10)), (-20, 2)), ('ej5', (di(-15, 30), m_(-15, 30)), (-10, F(1, 3))),
        ('ej6', (R_(float(di(10, 25)), 1), R_(float(m_(10, 25)))), (16.7, -0.67)), ('ej7', (di(5, 4), m_(5, 4)), (-20, 5)),
        ('ej8', (di(-12, 12), m_(-12, 12)), (-6, F(1, 2))),
        ('ej9', (R_(1/(1/0.1 - 1/3)*100, 1), round(-3/(1/(1/0.1 - 1/3)))), (10.3, -29)),
        ('ej10', (1/0.25, R_(1/-1.5)), (4.0, -0.67)),
    ]

KC, E_ = 8.99e9, 1.6e-19

@guia('PF19 Ley de Coulomb')
def _():
    k = KC
    F1 = k*3e-6*1e-6/0.16; F2 = k*3e-6*1e-6/0.25
    Fx, Fy = -F2*0.6, F1 + F2*0.8
    Gr = 6.67e-11*9.11e-31*1.67e-27/(5.3e-11)**2
    return [
        ('ejemplo1', 1e-6/E_, 6.25e12), ('ejemplo2', R_(k*2e-6*3e-6/0.05**2, 1), 21.6),
        ('hidrogeno', (R_(k*E_**2/(5.3e-11)**2*1e8, 1), R_(Gr*1e47, 1), round(math.log10(k*E_**2/(5.3e-11)**2/Gr))), (8.2, 3.6, 39)),
        ('ejemplo3', (R_(k*2e-6*1e-6/0.01), R_(k*3e-6*1e-6/0.04), R_(k*2e-6*1e-6/0.01 + k*3e-6*1e-6/0.04)), (1.80, 0.67, 2.47)),
        ('ej1', 8e-6/E_, 5e13), ('ej2', (5 - 1)/2, 2.0), ('ej3', R_(k*25e-12/0.01, 1), 22.5), ('ej4', round(k*24e-12/0.03**2), 240),
        ('ej5', (F(1, 4), 2*2), (F(1, 4), 4)), ('ej6', R_(math.sqrt(k*1e-12/0.9)), 0.10),
        ('ej7', sp.solve(sp.Eq(2*(sp.Rational(3, 10) - x), x), x), [sp.Rational(1, 5)]),
        ('ej8', (R_(F1, 3), R_(F2, 3), R_(Fx, 3), R_(Fy, 3), R_(math.hypot(Fx, Fy), 3), round(deg(math.atan2(Fy, Fx)))), (0.169, 0.108, -0.065, 0.255, 0.263, 104)),
    ]

@guia('PF20 Campo eléctrico')
def _():
    k = KC
    a = E_*1000/9.11e-31
    return [
        ('ejemplo1', 2e-9*5000, 1e-5), ('ejemplo2', R_(k*4e-6/0.04/1e5), 8.99),
        ('ejemplo3', (R_(k*2e-6/0.05**2/1e6), R_(2*k*2e-6/0.05**2/1e7)), (7.19, 1.44)),
        ('ejemplo4', (R_(a/1e14), R_(math.sqrt(2*a*0.01)/1e6)), (1.76, 1.87)),
        ('ej1', R_(0.6/3e-6, 6), 2e5), ('ej2', R_(E_*2e4*1e15, 6), 3.2), ('ej3', round(k*5e-9/0.09), 499),
        ('ej4', R_(math.sqrt(k*1e-6/9e4)), 0.32),
        ('ej5', (0, R_(k*3e-6/0.01/1e6), R_(k*3e-6/0.09/1e5, 1), R_((k*3e-6/0.01 - k*3e-6/0.09)/1e6)), (0, 2.70, 3.0, 2.40)),
        ('ej6', (R_(k*5e-6/0.01/1e6, 1), R_(2*k*5e-6/0.01/1e6)), (4.5, 8.99)), ('ej7', R_(E_*500/1.67e-27/1e10), 4.79),
        ('ej8', (R_(2e-15*9.8/30625*1e19, 6), round(2e-15*9.8/30625/E_)), (6.4, 4)),
    ]

@guia('PF21 Potencial eléctrico')
def _():
    k = KC
    return [
        ('ejemplo1', R_(k*(3e-6/0.1 - 2e-6/0.2)/1e5), 1.80),
        ('ejemplo2', (E_*1000, R_(math.sqrt(2*E_*1000/9.11e-31)/1e7), round(math.sqrt(2*E_*1000/9.11e-31)/3e8*100)), (1.6e-16, 1.87, 6)),
        ('ejemplo3', 200/0.05, 4000.0), ('ejemplo4', (10e-6*12, 0.5*1e-5*144), (1.2e-4, 7.2e-4)),
        ('placas', 8.85e-12*0.01/0.001, 8.85e-11),
        ('ej1', R_(k*2e-6/0.3/1e4, 1), 6.0), ('ej2', R_(-k*5e-9/0.5, 1), -89.9), ('ej3', R_(k*3e-6*4e-6/0.2), 0.54),
        ('ej4', k*4e-6/0.1 - k*4e-6/0.1, 0.0), ('ej5', 2e-6*50, 1e-4), ('ej6', R_(math.sqrt(2*E_*500/1.67e-27)/1e5, 1), 3.1),
        ('ej7', 50*E_, 8e-18), ('ej8', R_(1.5e5*0.002, 6), 300.0), ('ej9', (R_(470e-6*9*1e3, 6), R_(0.5*470e-6*81, 3)), (4.23, 0.019)),
        ('ej10', R_(8.85e-12*0.02/0.0005*1e10), 3.54),
    ]

@guia('PF22 Ley de Ohm y circuitos')
def _():
    par = lambda *r: 1/sum(1/v for v in r)
    A = math.pi*(0.5e-3)**2
    return [
        ('ejemplo1', (R_(A*1e7, 2), R_(1.72e-8*100/A)), (7.85, 2.19)),
        ('ejemplo2', (1500/120, 120**2/1500, 1.5*(20/60)*30), (12.5, 9.6, 15.0)),
        ('ejemplo3', (12/6, 2*4, 2*2, par(6, 3), 12/2, 12/6, 12/3), (2.0, 8, 4, 2.0, 6.0, 2.0, 4.0)),
        ('ejemplo4', (par(10, 15), R_(12/11), R_(12/11*6), R_(12/11*6/10), R_(12/11*6/15)), (6.0, 1.09, 6.55, 0.65, 0.44)),
        ('ej1', 3/0.5, 6.0), ('ej2', (120/10, 120*10), (12.0, 1200)), ('ej3', (60/120, 120**2/60), (0.5, 240.0)),
        ('ej4', (R_(0.2*100, 6), R_(0.2**2*100, 6)), (20.0, 4.0)), ('ej5', 2/2**2, 0.5), ('ej6', (0.15*720, 108*2.5), (108.0, 270.0)),
        ('ej7', (20/10, [2*r for r in (2, 3, 5)]), (2.0, [4, 6, 10])), ('ej8', (par(4, 12), 24/4, 24/12), (3.0, 6.0, 2.0)),
        ('ej9', R_(par(6, 6, 6), 9), 2.0), ('ej10', (R_(par(6, 3), 9), 8/(2 + 2), R_(4/6), R_(4/3)), (2.0, 2.0, 0.67, 1.33)),
    ]

@guia('PF23 Leyes de Kirchhoff')
def _():
    I1, I2, I3 = sp.symbols('I1 I2 I3')
    sol1 = sp.solve([I1 + I2 - I3, 10 - I1 - 2*I3, 5 - I2 - 2*I3], [I1, I2, I3])
    sol2 = sp.solve([I1 + I2 - I3, 16 - 2*I1 - 2*I3, 10 - I2 - 2*I3], [I1, I2, I3])
    return [
        ('nodo', 5 + 3 - 2, 6), ('ejemplo1', (9/4.5, 9 - 2*0.5), (2.0, 8.0)),
        ('ejemplo2', (sol1[I1], sol1[I2], sol1[I3]), (4, -1, 3)),
        ('ej2', 12/(1 + 2 + 3), 2.0), ('ej3', (1.5/3, R_(1.5 - 0.5*0.2, 6)), (0.5, 1.4)), ('ej4', (14 - 12)/0.5, 4.0),
        ('ej5', (sol2[I1], sol2[I2], sol2[I3]), (sp.Rational(7, 2), 1, sp.Rational(9, 2))),
        ('ej6', (3**2*2, 10*4, 4**2*1 + 5*1 + 1**2*1 + 18), (18, 40, 40)),
    ]

@guia('PF24 Campo magnético')
def _():
    mu = 4*math.pi*1e-7
    return [
        ('ejemplo1', (E_*2e6*0.5, R_(1.67e-27*2e6/(E_*0.5)*100, 1)), (1.6e-13, 4.2)), ('ejemplo2', R_(0.2*5*0.3, 6), 0.3),
        ('ejemplo3', (R_(mu*10/(2*math.pi*0.05)*1e5, 6), R_(mu*2000*2*1e3, 1)), (4.0, 5.0)),
        ('ej1', (R_(E_*3e6*0.02*1e15, 6), R_(9.11e-31*3e6/(E_*0.02)*1e3)), (9.6, 0.85)), ('ej2', math.sin(0), 0.0),
        ('ej3', R_(E_*1e5*0.4*0.5*1e15, 6), 3.2), ('ej4', R_(2*math.pi*1.67e-27/(E_*0.5)*1e7), 1.31),
        ('ej5', (R_(0.5*8*0.2, 6), R_(0.5*8*0.2*0.5, 6)), (0.8, 0.4)), ('ej6', R_(mu*20/(2*math.pi*0.02)*1e4, 6), 2.0),
        ('ej7', R_(2e-7*15/5e-5, 6), 0.06), ('ej8', R_(mu*3/(2*0.05)*1e5), 3.77), ('ej9', R_(mu*(500/0.25)*4*1e2, 1), 1.0),
    ]

@guia('PF25 Inducción electromagnética')
def _():
    return [
        ('ejemplo1', R_(0.2*0.05, 6), 0.01), ('ejemplo2', R_(200*0.01*0.5/0.1, 6), 10.0),
        ('ejemplo3', (R_(0.2*0.5*4, 6), R_(0.4/2, 6), R_(0.2*0.2*0.5, 6), R_(0.02*4, 6), R_(0.2**2*2, 6)), (0.4, 0.2, 0.02, 0.08, 0.08)),
        ('ejemplo4', (1000*12/120, 120/12), (100.0, 10.0)), ('perdidas', (1/10)**2, 0.01),
        ('ej1', R_(0.4*0.02*math.cos(rad(60)), 6), 0.004), ('ej2', R_(50*(0.02 - 0.005)/0.05, 6), 15.0), ('ej3', R_(0.3*0.1/0.02, 6), 1.5),
        ('ej4', (R_(0.6*0.3*5, 6), R_(0.9/3, 6)), (0.9, 0.3)), ('ej5', R_(4e-5*30*250, 6), 0.3),
        ('ej6', 5000*220/10000, 110.0), ('ej7', (1270*5/127, R_(10/127, 3)), (50.0, 0.079)), ('ej8', 1/10**2, 0.01),
    ]
